...
function display(){
	document.getElementById("stage").innerHTML="But: <span class=\"blue\">"+totalScore+"</span>"
	document.getElementById("try").innerHTML= " Score: <span class=\"blue\">"+score+"</span>"+" Arrivée du Roi:<span class=\"red\">"+timer+"</span>"
	document.getElementById("displayImg").innerHTML=""
	...
	for(var i=0;i<=divnumb-1;i++){
		if(i<2){
			...
			A[i].v=5
			vals[i]=A[i]
			newtext1=document.createTextNode(A[i].x+"épées *"+A[i].y+ " Marteaux = ")
...
function values(event){
	if(timer>0){
			...
			for(const e of tab){
				...
				if(e.style.backgroundColor==="lightgreen"){
					...
					if(scoreB===20){
						...
						document.getElementById("displayImg").innerHTML=""
						document.getElementById("displayFigure").innerHTML=""
						newtext2=document.createTextNode("FÉLICITATIONS vous avez sauvé le Village en récupérant toutes les armes")
		...
		document.getElementById("displayImg").innerHTML=""
		newp2=document.createElement('p');
		newtext2=document.createTextNode("OHHH !! Le roi est arrivé avant que vous ayez fini de rassembler toutes les armes et il a détruit le village")
		newp2.appendChild(newtext2)
